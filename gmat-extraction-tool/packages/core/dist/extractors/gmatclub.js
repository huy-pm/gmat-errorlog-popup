// @ts-nocheck
import { getPracticeUrl } from '../utils/dom';
/**
    ...
    if (answersPopup) {
        ...
        rows.forEach(row => {
            ...
            if (text.includes("My Answer:")) {
                ...
                if (span)
                    selectedAnswer = span.textContent.trim();
            }
    ...
    if (answerStartIndex !== -1) {
        ...
        let jsonData = {
            ...
            "content": {
                ...
                "answerChoices": answerChoices.map(choice => cleanTrashContent(decodeHtmlEntities(choice.content.replace(/<[^>]*>/g, '').trim()))).filter(choice => choice.length > 0) // Filter out empty choices after cleanup
            }
        };
        return jsonData;
    }
...
//# sourceMappingURL=gmatclub.js.map