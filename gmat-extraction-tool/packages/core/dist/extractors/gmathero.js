...
function detectQuestionType() {
    ...
    if (document.querySelector('.dropdown-selection'))
        return 'di-gi';
    if (document.querySelector('.ir-msr'))
        return 'di-msr';
    if (document.querySelector('.ir-ta'))
        return 'di-ta';
    if (document.querySelector('.tpa-question'))
        return 'di-tpa';
    // Verbal Types (use panel structure)
    if (document.querySelector('#left-panel .passage'))
        return 'rc';
    ...
    if (hasQuestionStem && !hasPassage && !hasKaTeX)
        return 'cr';
...
function getSectionFromType(questionType) {
    ...
    if (questionType === 'quant')
        return 'quant';
    if (questionType === 'ds')
        return 'di'; // DS is under DI section
    if (questionType === 'cr' || questionType === 'rc')
        return 'verbal';
    if (questionType.startsWith('di-'))
        return 'di';
    return 'unknown';
}
/**
...
function extractGMATHeroDITA() {
    try {
        ...
        if (headers.length === 0)
            _logWarning('TA: No table headers extracted');
        // Table rows
        ...
        if (statements.length === 0)
            _logWarning('TA: No statements extracted from .yes-no-question');
        var metadata = extractGMATHeroMetadata();
    ...
    try {
        ...
        if (!dropdownSelection) {
            ...
            for (var ci = 0; ci < chunks.length; ci++) {
                ...
                if (cleanedText)
                    contentData.statements.push({ text: cleanedText, dropdowns: dropdowns });
            }
        }
...
function extractGMATHeroDITPA() {
    try {
        ...
        if (choiceLabels.length === 0)
            _logWarning('TPA: No column headers found');
        ...
        if (tpaRows.length === 0)
            _logWarning('TPA: No rows extracted from grid');
    ...
    if (tbody) {
        tbody.querySelectorAll('tr').forEach(function (tr) {
            ...
            if (cells.length === 1 && colspan >= 2) {
                ...
                if (rowData.length > 0)
                    result.rows.push({ type: 'data', cells: rowData });
            }
        });
    }
    ...
    for (var i = 0; i < tabHeaders.length; i++) {
        ...
        if (textDiv) {
            ...
            if (tableInClone) {
                ...
                if (prevSib && (prevSib.querySelector('strong') || prevSib.tagName === 'STRONG'))
                    prevSib.remove();
                tableInClone.remove();
            }
...
async function ensureMsrAnswerRevealed() {
    ...
    var hasCorrect = function () {
        return !!rightPanel.querySelector('.standard-choices .correct-answer, .yes-no-question .correct-answer');
    };
    if (hasCorrect())
        return;
    ...
    if (yesNoQuestion) {
        ...
        standardChoices.querySelectorAll('.option').forEach(function (option) {
            ...
            if (!isCorrect && !standardChoices.querySelector('.correct-answer')) {
                ...
                isCorrect = !!((input && input.getAttribute('aria-checked') === 'true') ||
...
//# sourceMappingURL=gmathero.js.map