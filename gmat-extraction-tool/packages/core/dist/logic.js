import { sectionMappings, difficultyMappings, sourceMappings, allSectionMappings, allDifficultyMappings, detectSourceFromLink } from './utils/dom.js';
export function parseNotes(notes, categories) {
    ...
    for (const cat of categories) {
        ...
        if (categoryWords.length > 1) {
            for (let i = 0; i <= words.length - categoryWords.length; i++) {
                ...
                if (wordIndices.some(idx => usedWords.has(idx)))
                    continue;
                const matchesCategory = categoryWords.every((catWord, idx) => words[i + idx] === catWord);
                if (matchesCategory) {
                    category = cat.name;
                    if (!section) {
                        section = cat.section;
    ...
    if (!category) {
        for (let i = 0; i < words.length; i++) {
            if (usedWords.has(i))
                continue;
            const word = words[i];
            const categoryByShort = categories.find(cat => cat.shortName?.toLowerCase() === word);
            if (categoryByShort) {
...
export function parseNotesAndLink(notes, questionLink, categories) {
...
export function getAutoSuggestions(input, cursorPosition, parsedInfo, categories) {
...
export function applySuggestion(input, suggestion, cursorPosition) {
