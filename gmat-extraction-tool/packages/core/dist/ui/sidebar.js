...
import { ICONS, createBadge, showStatus, getPracticeUrl, enrichquestionData } from '../utils/dom.js';
import { parseNotesAndLink, getAutoSuggestions, applySuggestion } from '../logic.js';
import { fetchCategories, fetchTags, submitLog } from '../api.js';
import { hasValidToken, authenticate, clearToken } from '../auth-stub.js';
// ============================================================================
// UI HELPER FUNCTIONS
// ============================================================================
...
function updateParsedPreview(questionLink, notes, root) {
    ...
    if (parsed.source) {
        ...
        if (questionLink.trim())
            sourceBadge.innerHTML += ' <span style="margin-left:4px">📎</span>';
    ...
    if (parsed.category)
        badgesDiv.appendChild(createBadge(`Category: ${parsed.category}`, 'default'));
    if (parsed.difficulty)
        badgesDiv.appendChild(createBadge(`Difficulty: ${parsed.difficulty.charAt(0).toUpperCase() + parsed.difficulty.slice(1)}`, 'default'));
...
async function submitQuestionDataMain(root) {
    if (state.isSubmitting)
        return;
    state.isSubmitting = true;
    const questionLinkInput = root.getElementById('gmat-question-link');
    const notesInput = root.getElementById('gmat-notes');
...
function updateSidebarLayout() {
    if (state.isCollapsed) {
        ...
        if (state.activeTab === 'log') {
            setTimeout(() => {
                const notes = shadow.getElementById('gmat-notes');
                if (notes) {
                    notes.focus();
...
async function updateAuthUI(container) {
    ...
    if (isAuthenticated) {
        ...
        if (logoutBtn) {
            logoutBtn.addEventListener('click', () => {
                clearToken();
                showStatus('Signed out successfully', 'success', container.getRootNode());
                updateAuthUI(container);
        ...
        if (loginBtn) {
            loginBtn.addEventListener('click', async () => {
                showStatus('Opening login popup...', 'default', container.getRootNode());
                const success = await authenticate();
                if (success) {
...
async function updateHeaderAuthIcon(root) {
...
async function handleAnalysis() {
    ...
    try {
        ...
        if (analysis.detectedSource)
            state.logData.source = analysis.detectedSource;
    }
    catch (error) {
...
// LOGIN SCREEN
// ============================================================================
function renderLoginScreen(parent) {
...
function attachLoginEvents(root) {
    ...
    root.getElementById('btn-signin')?.addEventListener('click', async () => {
        const btn = root.getElementById('btn-signin');
        if (btn) {
            btn.disabled = true;
...
function renderTagList(tagsList, root) {
    tagsList.innerHTML = '';
...
function toggleTag(tag, tagsList, root) {
    const tags = state.logData.tags || [];
