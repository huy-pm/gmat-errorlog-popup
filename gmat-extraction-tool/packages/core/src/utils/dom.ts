...
// Determine base URL
// Note: In a service worker context, window might not be available.
const getBaseUrl = () => {
  if (typeof window !== 'undefined') {
    const isLocalhost = window.location.hostname === 'localhost' ||
      window.location.hostname === '127.0.0.1' ||
      window.location.href.includes('localhost');
    return CONFIG.devUrl; // Always devUrl based on previous logic: //isLocalhost ? CONFIG.devUrl : CONFIG.apiUrl;
  }
 ...
 */
export function getPracticeUrl(url: string): string {
...
export const ICONS = {
  ...
  lock: `<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>`
};

// Mappings for parsing
export const sectionMappings: Record<string, string> = {
  'vb': 'verbal',
  'verbal': 'verbal',
  'qt': 'quant',
  'quant': 'quant',
...
export const allSectionMappings: Record<string, string> = {
  'v': 'verbal',
  'vb': 'verbal',
  'verbal': 'verbal',
  'q': 'quant',
...
export const difficultyMappings: Record<string, string> = {
  'easy': 'easy',
  'med': 'medium',
  'medium': 'medium',
  'hard': 'hard'
...
export const allDifficultyMappings: Record<string, string> = {
  'e': 'easy',
  'easy': 'easy',
  'm': 'medium',
  'med': 'medium',
...
export const sourceMappings: Record<string, string> = {
  'og': 'gmat-og',
  'official': 'gmat-og',
...
export const sourceDisplayLabels: Record<string, string> = {
  'gmat-club': 'GMAT Club',
...
export function decodeHtmlEntities(text: string): string {
  if (typeof document === 'undefined') return text; // Fallback for non-browser env
 ...
 */
export function detectSourceFromLink(url: string | null): string | undefined {
 ...
 */
export function detectQuestionSource(url: string | null): string | null {
  if (!url) return null;
 ...
 */
export function createBadge(text: string, variant: 'green' | 'red' | 'default'): HTMLElement {
 ...
 */
export function showStatus(message: string, type: 'success' | 'error' | 'default', rootOrDoc: any = document): void {
  // Use any for rootOrDoc because TS might not know about ShadowRoot in some contexts, or it's strict.
  // Ideally Document | ShadowRoot
  const statusDiv = rootOrDoc.getElementById ? rootOrDoc.getElementById('gmat-logger-status') : null;
  if (!statusDiv) return;
 ...
 */
export function enrichquestionData(questionData: QuestionData | null, payload: any): QuestionData | null {
 ...
 */
export function isCompletionStyleQuestion(text: string | null): boolean {
