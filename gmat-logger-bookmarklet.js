...
(function() {
  ...
  const CONFIG = {
    ...
    version: '1.3.1' // Updated version to reflect draggable popup functionality
  };
  ...
  function setupEventListeners() {
    ...
    // Only close when clicking the overlay, not the dialog itself
    overlay.addEventListener('click', (e) => { 
      if (e.target === overlay) closeModal(); 
    });
    // Prevent dialog clicks from bubbling up to overlay
  ...
  function setupDragFunctionality() {
    ...
    // Remove the flex centering from the overlay
    ...
    header.addEventListener('mousedown', (e) => {
      ...
      // Prevent the click from bubbling up to modal
      e.stopPropagation();
      e.preventDefault();
    });
    ...
    document.addEventListener('mouseup', (e) => {
      if (isDragging) {
        ...
        // Prevent the mouseup from triggering modal close
        e.stopPropagation();
      }
    });
